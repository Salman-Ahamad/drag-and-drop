...
function drop(event) {
    event.preventDefault();
    let elementType;
    let newElement = null; // Initialize to null
    if (draggedElement) {
        elementType = draggedElement.getAttribute('data-content');
        // let newElement;
        switch (elementType) {
            case "div":
                newElement = createTagElement('div', true);  // The 'true' flag indicates that this tag should have an inner "+" button.
                break;
            case "h1":
                newElement = createTagElement('h1');
                break;
            case "button":
                newElement = createTagElement('button');
                break;
            case "function":
                newElement = createFunctionElement();
                break;
            case "array":
                newElement = createTagElement('array', true);
                break;
